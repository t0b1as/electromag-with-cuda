...
unsigned long CudaManager::ThreadFunctor(FunctorParams *params)
{
    ...
    errCode = cuCtxCreate(&ctx, CU_CTX_BLOCKING_SYNC , devices[params->GPUindex]);
...
int CudaManager::cuGpuDeviceGetProperties(CUDeviceProp *prop, CUdevice dev)
{
    CUresult result;
