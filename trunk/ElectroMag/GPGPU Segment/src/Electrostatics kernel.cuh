								   ...
								   unsigned int p, unsigned int fieldIndex, double resolution)
{
	...
	// Load starting point
	// The field vectors are arrannged as structure of arrays in order to enable coalesced reads
	// The x and y coordinates are interleaved in one array, producing coalesced 128-byte reads,
	// and the z coordinates are placed in a separate array, producing coalesced 64-byte reads
	...
	for(unsigned int step = 0; step < steps; step++)
	{
		...
		for(unsigned int i = 0; i < BLOCK_X; i++)
		{
			vec3Addto(temp, CoreFunctor(charge[i], point) );	// ElectroPartFieldFLOP + 3 FLOPs
		}
	}
	// Finally, add the unit vector of the field divided by the resolution to the previous point to get the next point
	point = vec3Div(vec3Add(point, vec3Unit(temp) ), resolution);// 15 FLOPs (3 div + 3 add + 9 unit)
	// The results must be written back as interleaved xy and separate z coordinates
	ptXY.x = point.x;
	ptXY.y = point.y;
	xyInterleaved[xyPitchOffset * fieldIndex + ti] = ptXY;
