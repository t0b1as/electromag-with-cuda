...
CUresult CudaElectrosFunctor<T>::AllocateGpuResources(size_t deviceID)
{
	...
	/*if(!blockMultiplicity)*/ size_t blockMultiplicity = (size_t)mpcount;
	...
	)
{
    ...
    __int64 FLOPs = ((__int64) (steps - 1) * (useCurvature ? CalcField_kernelFLOP_Curvature(params->elements, p) : CalcField_kernelFLOP(params->elements, p)));
